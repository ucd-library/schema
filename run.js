...
const folders = [indexFolder, schemasFolder, contextFolder, "./aggie_experts" ]

const Copy = { './aggie_experts': true,
               './_contexts/': true
             };
/**
...
function keywordParsing (folders){
  for(let folder of folders){
    fs.readdirSync(folder).forEach(file => {
      ...
      if ( Copy[folder] &&
           ( file.split('.').pop() == "json" || file.split('.').pop() == "jsonld")
         ) {
        fs.mkdirSync(path.join(tempRoot,folder),{recursive: true});
        let org = path.join(folder,file);
        let dest = path.join(tempRoot,org);
